...
parent: mainWindow, modal: true, webPreferences: {nodeIntegration:false} });
	var facebookAuthURL = 'https://www.facebook.com/v2.8/dialog/oauth?client_id=' + options.client_id +
'&redirect_uri=' + options.redirect_uri + 
'&response_type=token,granted_scopes&scope=' + 
options.scopes + '&display=popup';
	...
	authWindow.webContents.on('did-get-redirect-request', function (event, oldUrl, newUrl) {
		...
		if(access_token) {
			...
			FB.api('/me', { fields: ['id', 'name', 'picture.width(800).height(800)'] }, function (res) {                   
				mainWindow.webContents.send('fb-res', {
                					res: res,
